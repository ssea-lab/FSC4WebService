...
def _compute_task_idf(data, vocab_size, classes=None):
    '''
        Compute task specific idf
    '''
    ...
    if classes is None:
        classes = np.unique(data['label'])
    n_t = np.zeros(vocab_size, dtype=np.float32)
    for key in classes:
        n_t[n_d[key][0]] += n_d[key][1]

    idf = np.log(len(classes) / (0.1 + n_t))
    idf[idf<0] = 0
    idf = np.expand_dims(idf, axis=1)  # convert to 2d
...
def precompute_stats(train_data, val_data, test_data, args):
    ...
    if args.embedding in ['iwf', 'meta', 'meta_mlp','lstmatt']:
...
def get_task_idf(data, vocab_size):
    return _compute_task_idf(data, vocab_size)
