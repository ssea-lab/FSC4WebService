...
def _get_api_classes(args):
    if args.dataset == 'pw':
        a,b = 119,264 
    else:
        a,b = 45,96
    val_classes = list(range(0,a,2))
    test_classes = list(range(1,a,2))
    train_classes = list(range(a,b))
...
def _del_by_idx(array_list, idx, axis):
    ...
    for i, array in enumerate(array_list):
        if isinstance(array_list[i],list) and not isinstance(array_list[i][0],int):
            tmp = []
            for index,item in enumerate(array_list[i]):
                if index not in idx:
                    tmp.append(item)
            array_list[i] = tmp
        else:
...
def load_dataset(args):
    if args.dataset in ['pw','aws']:
        train_classes, val_classes, test_classes = _get_api_classes(args)
    else:
        raise ValueError(
